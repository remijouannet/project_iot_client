...
int protocole::send_query(char *data, char *password, int query, int *len1){
    *len1 = 0;
...
int protocole::send_reponse(char *data, char *message, int query, int *len1){
    *len1 = 0;
    *len1 += sprintf(data+*len1, "version: %d\r\n", VERSION);
    *len1 += sprintf(data+*len1, "time: %lu\r\n", time(NULL));
    *len1 += sprintf(data+*len1, "query: %d", query);
...
int protocole::parse_query(char *data, protocole::envoi *envoi_data){
    int check;
    check = sscanf(data, "version: %d time: %lu password: %s query: %d",
        &(envoi_data->version),
        &(envoi_data->time),
        (char*)&(envoi_data->password),
        &(envoi_data->query)
        );
...
int protocole::parse_reponse(char *data, protocole::reponse *reponse_data){
    int check;
...
int protocole::test() {
    ...
    protocole::envoi yolo;
    protocole::reponse yolo2;
